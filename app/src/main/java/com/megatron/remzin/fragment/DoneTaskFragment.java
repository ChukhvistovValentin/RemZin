...
import com.megatron.remzin.adapter.DoneTasksAdapter;
...
public class DoneTaskFragment extends TaskFragment {
    ...
    public void findTasks(String title) {
        ...
        tasks.addAll(this.activity.dbHelper.query().getTasks("task_status = ? AND task_title LIKE ?",
                new String[]{Integer.toString(2), "%" + title + "%"}, DBHelper.TASK_DATE_COLUMN));
    ...
    public void addTaskFromDB() {
        ...
        this.adapter.removeAllItems();
        List<ModelTask> tasks = new ArrayList();
        tasks.addAll(this.activity.dbHelper.query().getTasks(DBHelper.SELECTION_STATUS,
                new String[]{Integer.toString(2)}, DBHelper.TASK_DATE_COLUMN));
        for (int i = 0; i < tasks.size(); i++) {
            addTask((ModelTask) tasks.get(i), false);
        }
    }
...
//            this.alarmHelper.setAlarm(task);
    ...
    public void setGreenIcon() {
        DoneTasksAdapter.setGreenIcon_();
    }
}
